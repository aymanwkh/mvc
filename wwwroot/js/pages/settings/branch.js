const { onMounted, ref, shallowRef, toRef } = Vue
const setup = () => {
  ...
  const subtitle = toRef(() => `${isEditing.value ? 'Update' : 'Create'} your favorite book`)
  const title = toRef(() => `${isEditing.value ? 'Edit' : 'Add'} a Book`)
  const headers = [
  ...
  return {
    createNewRecord, books, formModel, dialog, isEditing, headers, add, edit, remove, reset, save, subtitle, title
  }
}
const template = /*html*/`
